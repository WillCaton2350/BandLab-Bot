...
from Bot.States.data import dm_username_xpath, user_specific_xpath
...
from Bot.States.data import direct_msg_xpath
from selenium.webdriver.common.by import By
from Bot.States.data import msg_text_xpath
from selenium import webdriver as web
from Bot.States.data import send_btn
from States.data import foryou_xpath
...
class web_driver:
    ...
    def feed_interaction(self):
        ...
        f'/html/body/main/div/section/div[2]/div[2]/div[3]/div/div[{post_number}]/post-card/div/post-tile-social/div/div[1]/div'
                ...
                if dm_btn.is_displayed() and dm_btn.is_enabled():
                    dm_btn.click()
                    self.driver.implicitly_wait(1)
                    sleep(1)
                    self.driver.find_element(
                            By.XPATH, direct_msg_xpath).click()
                    self.driver.implicitly_wait(1)
                    try:
                        WDW(
                            self.driver,10).until(
                                EC.presence_of_element_located((
                                    By.XPATH,dm_username_xpath
                                    )))
                    except NoSuchElementException as err:
                        print(err)
                    self.driver.find_element(
                    ...
                    self.driver.find_element(
                        By.XPATH,
                            user_specific_xpath).click()
                    self.driver.find_element(
                        By.XPATH,
                            msg_text_xpath
                        ).send_keys(
                    "Check my new single on platforms like Spotify, Soundcloud and more....")
                    self.driver.implicitly_wait(1)
                    sleep(1)
                    ...
                    print(f"sent {post_number} dm")
                else:
                    print(f"dm button for message {post_number} not clickable")
            except ElementNotInteractableException as err:
                print(err)
            try:
